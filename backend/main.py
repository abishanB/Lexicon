...
logger = logging.getLogger("lingualens-backend")
...
def get_fr_to_en_translation():
    ...
    from_language = next((language for language in installed_languages if language.code == "fr"), None)
...
async def lifespan(app: FastAPI):
    logger.info("Starting LinguaLens translation backend")

    translation = get_fr_to_en_translation()
    installed_codes = [language.code for language in translate.get_installed_languages()]
    ...
    if translation is None:
        ...
        logger.info("Sample translation check: %s -> %s", sample_text, sample_translation)

    yield

    logger.info("Stopping LinguaLens translation backend")


app = FastAPI(title="LinguaLens Translation Backend", lifespan=lifespan)
